...
global.getKeyakiDocument = function(e: any) {
    const date: string = '2019-12-01';
    const requestDate: string = date.replace(/-/gi, '');
    const url: string = "https://us-central1-augc-260709.cloudfunctions.net/getKeyakiSchedule?date=" + requestDate;
    const scheduleJson: string = UrlFetchApp.fetch(url).getContentText();
    // const scheduleJson = mockScheduleJson(); // API通信せずにmockする時に使う
    ...
    // その月のメディアの予定全部消す
    const targetDateBeginningOfMonth = new Date(date);
